...
lm.utils.copy( lm.items.RowOrColumn.prototype, {
	...
	_calculateAbsoluteSizes: function() {
		...
		for( i = 0; i < this.contentItems.length; i++ ) {
			if( this._isColumn ) {
				itemSize = ( totalHeight * ( this.contentItems[ i ].config.height / 100 ) );
			} else {
				itemSize = ( totalWidth * (this.contentItems[ i ].config.width / 100) );
			}
	...
	_respectMinItemWidth: function() {
		...
		//console.log("totalovermin: "+totalOverMin);
		...
		for( i = 0; i < entriesOverMin.length; i++ ) {
			...
			//console.log(( entry.width - minItemWidth ) * reducePercent);
			//console.log("REDUCED WIDTH: " + (reducedWidth) );
		...
		for( i = 0; i < this.contentItems.length; i++ ) {
			...
			//console.log((allEntries[ i ].width / sizeData.totalWidth) * 100);
		}
	},
	...
	_getMinimumDimensions: function( arr ) {
		...
		for( var i = 0; i < arr.length; ++i ) {
			minWidth = ( arr[ i ].minWidth || 0, minWidth );
			minHeight = ( arr[ i ].minHeight || 0, minHeight );
		}
		//console.log("--- min dimensions ----");
...
lm.utils.copy( lm.items.Stack.prototype, {
	...
	setSize: function() {
		var i,
			...
			contentWidth = parseFloat(this.element.width()) - (this._sided ? headerSize : 0),
			contentHeight = this.element.height() - (!this._sided ? headerSize : 0);

			//console.log(parseFloat(this.element.width()));

			//console.log("---- setting size -----");
			//console.log(contentWidth);
			//console.log((this._sided ? headerSize : 0));
